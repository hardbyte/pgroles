...
- **Object privileges**: a `REVOKE` that matches no entry the executor can act as **succeeds silently without changing anything** — no error, no warning. A privilege granted onward by a delegate therefore cannot be revoked by the pgroles executor. `REVOKE ... GRANTED BY` for object privileges must name the current user, so the way to remove a delegated grant is to revoke the delegate's grant option with `CASCADE`, or run the revoke as a role that can act as the grantor.
- **Role memberships** (PostgreSQL 16+): each membership edge records its grantor, and a bare `REVOKE role FROM member` removes only the edge attributed to the executor — with `ADMIN OPTION` but a different grantor it succeeds with just a `WARNING` ("role ... has not been granted membership ... by role ...") and the membership survives. `REVOKE ... GRANTED BY grantor` removes the right edge, but requires the privileges of that grantor; `CASCADE` does not bypass this — it only follows dependencies from a grant you are authorised to revoke.
