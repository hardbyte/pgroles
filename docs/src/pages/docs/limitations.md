...
Delegated grants also limit what a planned `REVOKE` can do: PostgreSQL only removes ACL entries whose grantor is a role the executor can act as, and a `REVOKE` that matches none of them **succeeds silently without changing anything** — no error, no warning. A privilege granted onward by a delegate therefore cannot be revoked by the pgroles executor. The plan preflight explodes the live ACLs behind every planned revoke and reports each entry whose grantor the executor cannot act as (`UnsatisfiableRevoke`, warning on `diff`, blocking on `apply`), and the operator additionally detects the residual case no preflight can prove — a plan whose exact effects were just applied yet still diff — reporting `Ready=False` with reason `NonConvergentPlan` instead of re-applying it in a loop. The fix is to revoke the delegating grant option (with `CASCADE`) or run the revoke as a role that can act as the grantor.
