...
Delegated grants also limit what a planned `REVOKE` can do, because PostgreSQL attributes every grant to a grantor and a revoke removes only grants attributable to the executor (see the upstream [REVOKE](https://www.postgresql.org/docs/current/sql-revoke.html) and [GRANT](https://www.postgresql.org/docs/current/sql-grant.html) documentation):

- **Object privileges**: "a user can only revoke privileges that were granted directly by that user", and a superuser's revoke "is performed as though it were issued by the owner of the affected object" — so a plain `REVOKE` that matches no such entry **succeeds silently without changing anything**, and a privilege granted onward by a delegate survives everyone's plain revoke, **superusers included**. `GRANTED BY` for object privileges must name the current user (`ERROR: grantor must be current user`), so the way to remove a delegated grant is to revoke the delegate's grant option with `CASCADE`, or run the revoke as the delegate.
- **Role memberships** (PostgreSQL 16+): each membership edge records its grantor, and a bare `REVOKE role FROM member` removes only the edge the revoke is attributed to — the executor under direct `ADMIN OPTION`, or for superusers the bootstrap superuser ("since roles do not have owners ... the command is performed as though it were issued by the bootstrap superuser"). Any other edge survives with just a `WARNING` ("role ... has not been granted membership ... by role ..."), even for superusers. `REVOKE ... GRANTED BY grantor` removes the right edge, but requires the privileges of that grantor; `CASCADE` does not bypass attribution — it only follows dependencies from a grant you are authorised to revoke.

The plan preflight checks both cases against the live catalogs and reports each entry or edge whose grantor the executor cannot act as (`UnsatisfiableRevoke`: warning on `diff`, blocking on `apply`), naming the grantor. The operator additionally detects the residual case no preflight can prove — a plan whose exact effects were just applied yet still diff — reporting `Ready=False` with reason `NonConvergentPlan` instead of re-applying it in a loop.
