...
| ` spec.roles[] ` | **object; item or branch.** A concrete role definition (CRD-compatible version). **Constraints:** ` {"required":["name"]} `. |
...
| ` spec.schemas[] ` | **object; item or branch.** A schema binding — associates a schema with one or more profiles.  Field bounds come from &#91;&#96;crate::bounds&#96;&#93;; see that module for why they exist and where else they are enforced. **Constraints:** ` {"required":["name"]} `. |
