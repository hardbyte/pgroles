...
| Path | Definition |
| --- | --- |
| `spec` | **object; required.** A GitOps-managed bundle of PostgreSQL memberships that may be requested. |
| `spec.approval` | **object; required.** Whether activation requires a recorded approval decision. |
...
| `spec.justification` | **object; required.** Whether requests must explain why access is needed. |
...
| `spec.memberships[]` | **object; item or branch.** Constraints on this array item, map value, or conditional schema. |
...
| `spec.postgresPolicyRef` | **object; required.** PostgresPolicy in this namespace that manages the target database. |
