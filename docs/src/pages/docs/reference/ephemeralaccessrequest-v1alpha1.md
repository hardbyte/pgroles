...
| Path | Definition |
| --- | --- |
| ` spec ` | **object; required.** One immutable runtime request for a bounded access bundle. **Constraints:** ` {"required":["accessPolicyRef","requestedBy","subject"]} `. |
...
| ` spec.accessPolicyRef ` | **object; required.** EphemeralAccessPolicy in this namespace that defines the requested bundle. **Constraints:** ` {"required":["name"]} `. |
...
| ` spec.requestedBy ` | **object; required.** Kubernetes identity which created the request. The supplied Kyverno reference policy overwrites this from authenticated admission &#96;userInfo&#96;. **Constraints:** ` {"required":["username"]} `. |
...
| ` spec.subject ` | **object; required.** PostgreSQL role receiving the temporary memberships. **Constraints:** ` {"required":["role"]} `. |
...
| ` status.decidedBy ` | **object; optional.** Kubernetes identity which approved or denied the request. The supplied Kyverno reference policy overwrites this from authenticated admission &#96;userInfo&#96; in the same status update as the terminal decision. **Constraints:** ` {"nullable":true,"required":["username"]} `. |
...
| ` status.resolvedAccess ` | **object; optional.** Write-once snapshot of the resolved target, duration, and exact memberships, recorded before approval so the decision binds to this access bundle. **Constraints:** ` {"nullable":true,"required":["accessPolicyGeneration","accessPolicyUid","bundleEncoding","bundleHash","grantedDuration","memberships","targetDatabaseFingerprint","targetPolicyGeneration","targetPolicyUid"]} `. |
