...
- The chart-less Kyverno plan-decision policy no longer ships a hardcoded operator ServiceAccount. `k8s/security/plan-decision-kyverno.yaml` carries the exemption subject as a placeholder and is rendered by `scripts/render-kyverno-policies.sh --namespace … --service-account …`, which refuses to emit a manifest with the placeholder unresolved or an empty subject. A default was wrong in both directions: it stalls plans when it names an account the operator does not run as, and it hands the approve-verb check a bypass to anyone who can create that account in a cluster that installs the operator elsewhere. The Helm path already generated the subject from `operator.serviceAccount.name` and the release namespace, and now fails the render rather than emitting an unsubstituted policy. (#187, #179)
