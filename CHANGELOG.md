...
- **Reconcile concurrency is bounded, so peak memory no longer scales with how many policies come due at once.** kube-rs defaults `Config::concurrency` to `0` — unbounded — so every controller reconciled every object whose watch fired, immediately. Reconciling a `PostgresPolicy` inspects the whole managed surface of its database, and the function-ACL read is one `aclexplode` row per function per grantee: ~102k rows on a policy covering 49 schemas and 32 roles. Peak allocation was therefore a function of how many policies happened to be due simultaneously rather than the size of any one of them, and on startup every watch fires together. An operator watching 28 such policies was OOM-killed 31 seconds after each start at a 1Gi limit, restarting 1709 times over five days in CrashLoopBackOff and never advancing `observed_generation`; a second watching 6 showed the same signature at 1385 restarts, while one watching 4 has never restarted and sits at 9Mi. Raising the first operator's ceiling to 2Gi settles it at 290Mi, so the steady-state working set is modest and the failure is the simultaneous burst. Concurrency now defaults to 4 across the policy, access-policy and access-request controllers, and is operator-level configuration via `RECONCILE_CONCURRENCY`; `0` restores the previous unbounded behaviour, and an invalid value refuses startup with the variable named, as the `PLAN_RETENTION_*` bounds already do. A bound also sits closer to what the database can absorb: pools are capped at 5 connections per database and a policy already holds an advisory lock for its own, so reconciles beyond a handful queue on connections rather than progress.
