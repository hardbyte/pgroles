...
- **Revokes now target each grant's recorded grantor, so delegated grants converge.** PostgreSQL's [`REVOKE`](https://www.postgresql.org/docs/current/sql-revoke.html) removes only grants attributed to the revoker (a superuser acts as the object owner, or the bootstrap superuser for role memberships), so a privilege granted onward by a delegate — or a membership edge granted by another admin — survived every plain revoke, superuser executors included, and re-planned forever. Inspection now records the grantor of every ACL entry and membership edge (one role/member pair can carry several edges since PostgreSQL 16; they are aggregated for diffing and each edge is revoked individually), and rendered revokes act as that grantor: `SET ROLE grantor; REVOKE ...;` (restoring the connection's configured execution role afterwards) for object privileges, `REVOKE ... GRANTED BY grantor` for memberships. Delegated grants to `PUBLIC` are attributed and converge the same way. The preflight verifies per grantor that the executor can become it (membership with the `SET` option for objects, the grantor's privileges for memberships; superusers qualify everywhere) and reports what is missing (`UnsatisfiableRevoke`): `diff` warns, `apply` blocks. Object-ACL grantors are recorded on every supported version, so object revokes are grantor-targeted everywhere; membership targeting requires the per-edge grantor PostgreSQL 16 records, and older servers keep plain membership revokes. Object revokes run before membership removals, and the preflight checks grantor authority phase by phase: a plan whose membership removals would strip the executor's path to a grantor a later statement still needs is flagged, while default-privilege revokes — which run after the plan's own membership additions — are judged with those additions counted, so a path the plan replaces or newly acquires is not falsely rejected. The `grantor` field is omitted from serialized changes when absent, so digests and JSON output of grantor-less plans are unchanged. See [limitations](https://thepartly.github.io/pgroles/docs/limitations/#grant-options-and-effective-access).

### Fixed

- **The operator no longer re-applies a non-convergent plan several times a second.** A plan whose SQL succeeds without changing inspected state (e.g. an unenforceable revoke) re-applied about once a second under `approval: auto`, because each replan minted a fresh plan and every plan status write woke the controller. The reconciler now detects that identical effects were just applied and still diff, reports `Ready=False` with reason `NonConvergentPlan`, and retries at the policy interval.
- **`password_valid_until` now converges.** Offsets and fractional seconds could never compare equal to the inspected UTC whole-second rendering, and `VALID UNTIL 'infinity'`/`'-infinity'` inspected as an empty string — each re-planned an `ALTER ROLE` on every run. `infinity` now inspects as "no expiration", `-infinity` stays distinguishable so one apply repairs it, and validation accepts exactly the convergent form `YYYY-MM-DDTHH:MM:SSZ`.
  **Upgrade:** an offset or fractional-second timestamp is now a field-level validation error — rewrite it in UTC whole seconds (it never converged before).
