...
- **The deprecated `PostgresPolicy` status fields `planned_sql`, `planned_sql_truncated`, and `last_reconcile_time` are gone.** They were superseded in 0.5.0 when `PostgresPolicyPlan` took over plan artifacts, but the operator kept writing them for backward compatibility — re-rendering and re-storing up to 16 KiB of SQL into policy status on every reconcile that had pending changes, duplicating what the plan object and its ConfigMap already held. **Migration:** read plan SQL from the plan the policy points at — `kubectl get pgplan $(kubectl get pgr <policy> -o jsonpath='{.status.current_plan_ref.name}') -o jsonpath='{.status.sqlInline}'` — and fall back to the gzipped ConfigMap named by `status.sqlRef` for plans above the inline limit. Replace `status.last_reconcile_time` with `status.last_successful_reconcile_time`, which has always carried the same value. Anything still reading the removed fields sees them absent rather than empty; no `PostgresPolicy` spec changes are required and no reconcile behavior changes. (#73)
