...
- **A `spec.mode: plan` policy now revalidates when drift disappears out of band.** Plan mode only ever opened plans; when the database converged by other means the pending plan stayed Pending and `current_plan_ref` kept pointing at it while the policy reported `InSync`. It now supersedes that plan and clears the reference. (#185)
